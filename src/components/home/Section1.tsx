...
const Section1 = () => {
    useEffect(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (!requestId) {
                requestId = requestAnimationFrame(animateTags);
            }
        };
        const animateTags = () => {
            tags.forEach((tag, index) => {
                const intensityMap = [0.01, 0.02, 0.03, 0.04, 0.02]; // 각 태그별 움직임 강도 설정
                const factor = intensityMap[index];
            ...
            requestId = 0; // 다음 애니메이션 요청을 받을 수 있도록 초기화
        };
